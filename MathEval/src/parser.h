...
namespace Lexer
{
	...
	*/
	...
	{
		ERROR_BIN_OP = -1, ADD_OP, SUB_OP, MULT_OP, DIV_OP,
	};
	...
	typedef struct tag_tree_node
	{
		...
		union
		{
			struct // ie: a+b
			{
				...
				bin_op op_type = bin_op::ERROR_BIN_OP;
			} binary_op;
		...
		tag_tree_node() {}
		~tag_tree_node() { if (type == node_type::PREFIX_OP && this->prefix_op.lexeme) delete this->prefix_op.lexeme; }
	} tree_node;
